...
def save_car_booking(data, id):
    rentalPoint = RentalPoint.query.filter(RentalPoint.id == data['car']).first()
    car = UserCar(
        from_date=data['from_date'],
        to_date=data['to_date'],
        price=0.0,
        reservation_number=rentalPoint.title,
        car_id=data['car'],
        owner_id=id
    )
    db.session.add(car)
...
def get_user_cars(id):
    data = []
    for car in UserCar.query.filter(UserCar.owner_id == id).all():
        data.append([car, get_user_data(car.owner_id)])
    return data
