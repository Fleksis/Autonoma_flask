...
def profile():
  try:
    if is_user_logged():
      return render_template("templates/profile.html", cars = get_cars(), user = get_user_data(session['user']), date = date.today().strftime("%Y-%m-%d"), rental_points = get_all_rental_points(), user_cars = get_user_cars(session['user']))
    else:
      return redirect(url_for('home'))
  except Exception as e:
    flash(str(e))
